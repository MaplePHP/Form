...
abstract class AbstractFormFields extends Arguments implements FormFieldsInterface {
	...
	public function container(callable $callback): string 
	{
		...
		if(!is_null($this->description)) $out .= "<div class=\"description legend\">{$this->description}</div>";
		$out .= $callback();
		$out .= "</div>";
		return $out;
	}
	...
	{
		if(isset($this->attrArr['data-clear'])) $this->attr(["required" => "required"]);
	...
	public function select(): string 
	{
		return $this->container(function() {
			...
			$name = $this->name;
			if(isset($this->attrArr['multiple'])) $name .= "[]";
